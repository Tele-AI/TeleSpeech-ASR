...
def Dataset(data_type,
    ...
    """
    assert data_type in ['raw', 'shard', 'ark', 'discrete']
    ...
    if data_type == 'shard':
        ...
        dataset = Processor(dataset, processor.parse_ark)
    elif data_type == 'raw':
        dataset = Processor(dataset, processor.parse_raw)
    else:
        dataset = Processor(dataset, processor.parse_discrete)

    dataset = Processor(dataset, processor.tokenize, symbol_table, bpe_model,
                        non_lang_syms, conf.get('split_with_space', False))
    
    resample_conf = conf.get('resample_conf', {})
    cmvn = conf.get('cmvn', False)
    feats_type = conf.get('feats_type', 'fbank')

    assert feats_type in ['fbank', 'mfcc', 'raw']

    if data_type == 'shard' or data_type == 'raw':
        ...
        filter_conf = conf.get('filter_conf', {})
        dataset = Processor(dataset, processor.filter_feat, **filter_conf)

    if cmvn:
        if data_type == 'ark':
            dataset = Processor(dataset, processor.cmvn)
        elif feats_type == 'raw':
            dataset = Processor(dataset, processor.cmvn_norm)
