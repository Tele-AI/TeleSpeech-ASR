...
def parse_ark(data):
    ...
    for sample in data:
        ...
        try:
            ...
            feat_ark = REGEX_FEAT.sub("", arr[1])
            mat = kaldiio.load_mat(feat_ark)
            with torch.no_grad():
                feat = torch.from_numpy(mat).float()
            shape = [int(x) for x in arr[2].split(":")[1].split(",")]
            num_frames = shape[0]
            example = dict(key=key,
                            txt=text,
                            feat=feat,
            ...
            logging.warning('Failed to read {}'.format(line))

def parse_discrete(data):
    for sample in data:
        assert 'src' in sample
        json_line = sample['src']
        obj = json.loads(json_line)
        ...
        key = obj['key']
        text = obj['txt']
        discrete_token = obj['token']
        array = [int(x) for x in discrete_token.split()]
        num_frames = len(array)
        discrete_token = torch.tensor(array)
        example = dict(key=key,
                        feat=discrete_token,
                        txt=text,
                        num_frames=num_frames,
...
def filter_feat(data,
    ...
    for sample in data:
        ...
        assert 'label' in sample
        assert 'num_frames' in sample, print('not ark feats!')
        # sample['feat'] is torch.Tensor, we have 100 frames every second
...
def compute_raw(data):
    for sample in data:
        ...
        assert 'label' in sample
        feat = sample['wav'].squeeze(0)
        yield dict(key=sample['key'], label=sample['label'], feat=feat)
...
def sort(data, sort_size=500):
    ...
    for sample in data:
        ...
        if len(buf) >= sort_size:
            buf.sort(key=lambda x: x['feat'].size(0))  # x= [T, D]
            for x in buf:
                yield x
            buf = []
    # The sample left over
