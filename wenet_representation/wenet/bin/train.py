...
from wenet.utils.init_model import init_model_with_feat_extractor
...
def get_args():
    ...
    parser.add_argument('--data_type',
                        default='raw',
                        choices=['raw', 'shard', 'ark', 'discrete'],
...
def main():
    ...
    if 'preencoder_conf' in configs:
        input_dim = configs['preencoder_conf']['output_size']
    elif 'frontend_conf' in configs and configs['frontend_conf']['feat_extractor_type'] != 'None':
        input_dim = configs['frontend_conf']['output_dim']
    elif configs['dataset_conf']['feats_type'] == 'log_mel_spectrogram':
        input_dim = configs['dataset_conf']['log_mel_spectrogram_conf']['num_mel_bins']
    elif configs['dataset_conf']['feats_type'] == 'mfcc':
        assert 'mfcc_conf' in configs['dataset_conf']
    ...
    model = init_model_with_feat_extractor(configs)
    ...
    if 'freeze_param' in configs:
        for t in configs['freeze_param']:
            for k, p in model.named_parameters():
                if k.startswith(t + ".") or k == t:
                    p.requires_grad = False

    if args.rank == 0:
        logging.info("==================params that need update======================")
        for name, param in model.named_parameters():
            if param.requires_grad:
                logging.info(f"{name} need updates")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        total_loss, num_seen_utts, total_cer = executor.cv(model, cv_data_loader, device,
                                                configs)
        ...
        if args.rank == 0:
            ...
            writer.add_scalar('epoch/cer', total_cer, epoch)
        final_epoch = epoch
