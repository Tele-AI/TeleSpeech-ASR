        ...
        with model_context():
            for batch_idx, batch in enumerate(data_loader):
                ...
                if batch_idx % log_interval == 0:
                    ...
                    # logging.debug('cer {:.6f} '.format(c_err))
    ...
    def cv(self, model, data_loader, device, args):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(data_loader):
                ...
                loss = loss_dict['loss']
                total_err_char += loss_dict["err_char"]
                total_sum_tok += loss_dict["sum_tok"]
                ...
                if batch_idx % log_interval == 0:
                    ...
                    logging.debug(log_str)
            total_cer = "{:.2f}".format(100 * float(total_err_char) / total_sum_tok)
            log_str = 'CV epoch {} cer: {}'.format(epoch, total_cer)
            logging.debug(log_str)
        return total_loss, num_seen_utts, float(total_cer)
